...
import { api, fmt, todayLocal, type Location, type StockRow } from '../api'
...
export default function Stock() {
  ...
  const filtered = (rows.data ?? []).filter((r) =>
    !term || [r.albumTitle, r.versionName, r.skuName, r.barcode]
      .some((v) => v?.toLowerCase().includes(term)))
  const total = filtered.reduce((s, r) => s + r.qty, 0)

  function saveCsv() {
    // 검색 중이면 보이는 것만 내보내되, 파일명에 남겨 부분 파일임을 알 수 있게 한다
    downloadCsv(`재고현황_${todayLocal()}${term ? `_검색_${q.trim()}` : ''}.csv`,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <div className="w-56">
            <Input placeholder="앨범·SKU·바코드 검색" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
          <div className="w-48">
          ...
          <Button variant="ghost" disabled={filtered.length === 0} onClick={saveCsv}>엑셀 저장</Button>
        </div>
      </div>
      <Card>
